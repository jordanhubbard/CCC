...
fn build_catalog() -> Vec<(String, String, String, String)> {
    ...
    let mut entries = vec![
        (
            "acc-server".to_string(),
            "ACC Server".to_string(),
            format!("http://{public_host}:{ccc_port}/health"),
            hub.clone(),
        ),
        (
            "agentbus".to_string(),
            "AgentBus".to_string(),
            format!("http://127.0.0.1:{ccc_port}/api/health"),
            hub.clone(),
        ),
    ];
    // Optional Qdrant
...
async fn probe_all() -> Vec<Value> {
    ...
    let probes: Vec<_> = catalog
        .into_iter()
        .map(|(id, name, url, host)| {
