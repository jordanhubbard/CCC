...
async fn test_topology_has_required_fields() {
    let ts = helpers::TestServer::new().await;
    let body =
        helpers::body_json(helpers::call(&ts.app, helpers::get("/api/geek/topology")).await).await;
    assert!(body["nodes"].is_array(), "topology must have nodes array");
    assert!(body["edges"].is_array(), "topology must have edges array");
    assert!(
        body["heartbeatSummary"].is_array(),
        "topology must have heartbeatSummary array"
    );
    assert!(
        body["busMessages"].is_array(),
        "topology must have busMessages array"
    );
}
    ...
    for node in body["nodes"].as_array().unwrap() {
        if node["type"] == "agent" {
            assert!(
                ...
                "agent node must have a status field"
            );
        }
    }
}
...
async fn test_mesh_same_shape_as_topology() {
    let ts = helpers::TestServer::new().await;
    let body = helpers::body_json(helpers::call(&ts.app, helpers::get("/api/mesh")).await).await;
