...
async fn test_set_and_get_secret() {
    ...
    let get_body =
        helpers::body_json(helpers::call(&ts.app, helpers::get("/api/secrets/API_TOKEN")).await)
            .await;
...
async fn test_delete_secret() {
    ...
    let del_body =
        helpers::body_json(helpers::call(&ts.app, helpers::delete("/api/secrets/DEL_ME")).await)
            .await;
