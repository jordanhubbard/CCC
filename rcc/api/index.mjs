...
# RCC Agent Onboard — ${entry.agent} (hint-role: ${agentRole}) @ ${new Date().toISOString()}
# Generated by Rocky Command Center
# Usage: curl "${RCC_PUBLIC_URL}/api/onboard?token=<token>" | bash
set -euo pipefail

AGENT_NAME="${entry.agent}"
ROLE_HINT="${agentRole}"   # hint from token; may be overridden by auto-detection below
...
${deployKeyBlock}
# ════════════════════════════════════════════════════════════════════
#  STEP 1: HOST AUTO-DETECTION
#  Detect GPU, Tailscale, disk, and self-select role
# ════════════════════════════════════════════════════════════════════
echo "→ Detecting host capabilities..."

# GPU detection
DETECT_GPU_COUNT=0
DETECT_GPU_MODEL=""
DETECT_GPU_VRAM_GB=0
DETECT_HAS_GPU=false
if command -v nvidia-smi &>/dev/null && nvidia-smi &>/dev/null; then
  DETECT_GPU_COUNT=\$(nvidia-smi --list-gpus 2>/dev/null | wc -l || echo 0)
  DETECT_GPU_MODEL=\$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 | xargs || echo "unknown")
  DETECT_GPU_VRAM_GB=\$(nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits 2>/dev/null | awk '{sum+=\$1} END {printf "%d", sum/1024}' || echo 0)
  if [ "\$DETECT_GPU_COUNT" -gt 0 ]; then
    DETECT_HAS_GPU=true
    echo "  GPUs: \$DETECT_GPU_COUNT x \$DETECT_GPU_MODEL (\${DETECT_GPU_VRAM_GB} GB VRAM total)"
  fi
else
  echo "  GPUs: none detected (nvidia-smi not found)"
fi
...
if command -v tailscale &>/dev/null; then
  ...
  TS_SELF_IP=\$(echo "\$TS_STATUS" | python3 -c "
import json,sys
try:
  d=json.load(sys.stdin)
  print(d.get('Self',{}).get('TailscaleIPs',[None])[0] or '')
except: print('')
" 2>/dev/null || true)
  if [ -n "\$TS_SELF_IP" ]; then
    DETECT_TAILSCALE=true
    DETECT_TAILSCALE_IP="\$TS_SELF_IP"
    echo "  Tailscale: ✅ online at \$DETECT_TAILSCALE_IP"
  else
...
# Disk space (GB available at /tmp for model storage)
DETECT_DISK_FREE_GB=\$(df -BG /tmp 2>/dev/null | awk 'NR==2{gsub("G","",\$4); print \$4}' || echo 0)
echo "  Disk free at /tmp: \${DETECT_DISK_FREE_GB} GB"

# vLLM check (already installed?)
DETECT_VLLM_INSTALLED=false
if [ -f "\$HOME/.vllm-venv/bin/python3" ] && "\$HOME/.vllm-venv/bin/python3" -c "import vllm" 2>/dev/null; then
  DETECT_VLLM_INSTALLED=true
  echo "  vLLM: already installed"
fi

# ── Role self-selection ────────────────────────────────────────────────────
# Override role_hint if hardware clearly qualifies for vllm-worker
EFFECTIVE_ROLE="\$ROLE_HINT"
if [ "\$ROLE_HINT" = "agent" ] && [ "\$DETECT_HAS_GPU" = "true" ] && [ "\$DETECT_GPU_VRAM_GB" -ge 64 ]; then
  echo ""
  echo "  ⚡ Auto-upgrade: \${DETECT_GPU_VRAM_GB} GB VRAM detected → upgrading role to vllm-worker"
  EFFECTIVE_ROLE="vllm-worker"
fi
if [ "\$ROLE_HINT" = "vllm-worker" ] && [ "\$DETECT_HAS_GPU" = "false" ]; then
  echo ""
  echo "  ⚠️  Role hint was vllm-worker but no GPUs detected — falling back to agent"
  EFFECTIVE_ROLE="agent"
fi
echo "  Effective role: \$EFFECTIVE_ROLE"
echo ""
...
# ════════════════════════════════════════════════════════════════════
#  STEP 6: vLLM WORKER SETUP (only when EFFECTIVE_ROLE=vllm-worker)
# ════════════════════════════════════════════════════════════════════
if [ "\$EFFECTIVE_ROLE" = "vllm-worker" ]; then
${vllmBlock}
  # ── Register vLLM capabilities (includes GPU + tailscale) ───────────────
  echo "→ Registering vLLM capabilities with RCC..."
  ...
  curl -sf -X POST "\${RCC_URL}/api/agents/register" \\
    ...
    }" | python3 -c "import json,sys; d=json.load(sys.stdin); print('   ✅ registered (vllm-worker)' if d.get('ok') else '   ⚠️  register: ' + str(d))" 2>/dev/null || echo "   ⚠️  registration failed"

  # ── Tunnel setup (only if no Tailscale) ─────────────────────────────────
  if [ "\$DETECT_TAILSCALE" = "false" ]; then
    echo "→ No Tailscale — setting up SSH reverse tunnel..."
    RCC_TUNNEL_HOST="${RCC_PUBLIC_URL.replace(/^https?:\/\//, '').split(':')[0]}"
    TUNNEL_KEY="\$HOME/.ssh/rcc-tunnel-key"
    if [ ! -f "\$TUNNEL_KEY" ]; then
      ssh-keygen -t ed25519 -f "\$TUNNEL_KEY" -N "" -C "\${AGENT_NAME}-vllm-tunnel"
    ...
    TUNNEL_RESP=\$(curl -sf -X POST "\${RCC_URL}/api/tunnel/request" \\
      -H "Authorization: Bearer ${agentToken}" \\
      -H "Content-Type: application/json" \\
      -d "{\\"pubkey\\":\\"\${TUNNEL_PUBKEY}\\",\\"agent\\":\\"\${AGENT_NAME}\\",\\"label\\":\\"\${AGENT_NAME}-vllm-tunnel\\"}" 2>/dev/null || echo '{}')
    TUNNEL_PORT=\$(echo "\$TUNNEL_RESP" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('port',18082))" 2>/dev/null || echo "18082")
    echo "  ✅ Tunnel allocated on port \$TUNNEL_PORT"
...
TUNNELEOF
    ...
    echo "  ✅ Tailscale detected — no SSH tunnel needed (direct access at \$DETECT_TAILSCALE_IP:8080)"
  fi
fi  # end vllm-worker block

# ════════════════════════════════════════════════════════════════════
#  STEP 7: REGISTER (non-vllm path — gpu info still included if present)
# ════════════════════════════════════════════════════════════════════
if [ "\$EFFECTIVE_ROLE" != "vllm-worker" ]; then
  echo "→ Registering identity with RCC..."
  AGENT_HOSTNAME=\$(hostname)
  curl -s -X POST "${RCC_PUBLIC_URL}/api/agents/register" \\
    -H "Authorization: Bearer ${agentToken}" \\
    -H "Content-Type: application/json" \\
    -d "{
      \\"name\\": \\"\${AGENT_NAME}\\",
    ...
    }" | python3 -c "import json,sys; d=json.load(sys.stdin); print('   ✅ registered' if d.get('ok') else '   ⚠️  register: ' + str(d))" 2>/dev/null || echo "   ℹ️  (registration failed — check token)"
fi
...
sleep 2
AGENT_HOSTNAME=\$(hostname)
curl -s -X POST "${RCC_PUBLIC_URL}/api/heartbeat/$AGENT_NAME" \\
  -H "Authorization: Bearer ${agentToken}" \\
  -H "Content-Type: application/json" \\
  -d "{\\"agent\\":\\"$AGENT_NAME\\",\\"role\\":\\"\${EFFECTIVE_ROLE}\\",\\"host\\":\\"\${AGENT_HOSTNAME}\\",\\"status\\":\\"online\\",\\"pullRev\\":\\"\${PULL_REV}\\"}" | grep -q '"ok":true' && echo "   ✅ heartbeat posted" || echo "   ⚠️  heartbeat failed (agent may still be starting)"

echo ""
echo "✅ $AGENT_NAME is online."
echo "   Role:    \$EFFECTIVE_ROLE"
echo "   Host:    \$AGENT_HOSTNAME"
echo "   GPUs:    \${DETECT_GPU_COUNT} x \${DETECT_GPU_MODEL} (\${DETECT_GPU_VRAM_GB} GB)"
echo "   TS IP:   \${DETECT_TAILSCALE_IP:-none}"
echo "   Token:   ${agentToken}"
if [ "\$EFFECTIVE_ROLE" = "vllm-worker" ]; then
  echo ""
  echo "   vLLM checklist:"
  if [ "\$DETECT_TAILSCALE" = "true" ]; then
    echo "   1. ✅ Direct Tailscale access at \$DETECT_TAILSCALE_IP:8080"
  else
    echo "   1. Verify tunnel key added to RCC tunnel user"
    echo "   2. Check tunnel: systemctl --user status rcc-vllm-tunnel"
  fi
  echo "   3. Check vLLM: curl http://localhost:8080/v1/models"
  echo "   4. Check seeder: curl http://localhost:18081/filelist.txt | head"
fi
`;
